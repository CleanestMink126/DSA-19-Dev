...
public class Problems {
    ...
    public static double[] runningMedian(int[] inputStream) {
        ...
        for (int i = 0; i <= inputStream.length; i++) {
            ...
            boolean insert = aboveMedian.isEmpty() || value <= aboveMedian.peek();
            // Insert into one of the PQs
            boolean b = (insert) ? aboveMedian.offer(value):belowMedian.offer(value);
            // Correct the sizes of the PQs according to the rule above
            if (belowMedian.size() > aboveMedian.size()) aboveMedian.offer(belowMedian.poll());
            if (belowMedian.size() < aboveMedian.size()) belowMedian.offer(aboveMedian.poll());
            // Calculate the median
            if (belowMedian.size() > aboveMedian.size())
                runningMedian[i] = belowMedian.poll();
            else
                runningMedian[i] = (belowMedian.poll() + aboveMedian.poll()) / 2;
        }
